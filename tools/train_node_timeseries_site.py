...
def train(args):
    ...
    for network in networks:
        for ws in W_list:
            ...
            if args.regression:
                print(f"Best MAE for window {ws} = {best_test_mae_curr_fold} "
                        f"at epoch {best_test_epoch_curr_fold}")
                results.append([
                    network, '_'.join(args.ensemble_networks), 
                    ws, best_test_mae_curr_fold, best_test_epoch_curr_fold
                ])
            else:
                print(f"Best AUC for window {ws}  = {best_test_auc_curr_fold} "
                        f"at epoch {best_test_epoch_curr_fold}")
                results.append([
                    network, '_'.join(args.ensemble_networks), 
                    ws, best_test_auc_curr_fold, best_test_epoch_curr_fold
                ])
