...
from models.MSG3D.fcgcn import Model
...
def create_output_folder(network):
    """
...
def train(args):
    ...
    data_path = os.path.join(args.data_path, 'node_FC/node_FC')
    ...
    for network in networks:
        ...
        # Create a folder for logging/checkpoints
        folder_to_save_model = create_output_folder(network)
        ...
        print('\n' + '#' * 30)
        print(f'Starting training for Network: {network}')
        print('#' * 30 + '\n')
        ...
        # --- Load Data ---
        train_data = np.load(os.path.join(data_path, f'train_data.npy'))
        train_label = np.load(os.path.join(data_path, f'train_label.npy'))

        test_data = np.load(os.path.join(data_path, f'test_data.npy'))
        test_label = np.load(os.path.join(data_path, f'test_label.npy'))
        test_subjects = np.load(os.path.join(data_path, f'test_subjects.npy'))

        # Reshape data to [N, C, T, V, M] as needed by MSG3D
        # We keep the entire T (train_data.shape[2]) dimension
        train_data = train_data.reshape(train_data.shape[0], 1, train_data.shape[2], train_data.shape[3], 1)
        test_data  = test_data.reshape(test_data.shape[0], 1, test_data.shape[2], test_data.shape[3], 1)
        ...
        for epoch in range(num_epochs):
            ...
            # Here we do NOT slice over the time dimension. We use the entire time range
            # So the training batch is just the chosen subset of subjects, all frames:
            train_data_batch = train_data[idx_batch, :, :, :, :]
            ...
            # --- Validation step (single pass over test set) ---
            if epoch % args.epochs_val == 0:
                net.eval()
                prediction = np.zeros(test_data.shape[0])

                # Single pass over the test set
                for start_idx in range(0, test_data.shape[0], batch_size_testing):
                    end_idx = min(start_idx + batch_size_testing, test_data.shape[0])
                    batch_indices = list(range(start_idx, end_idx))

                    test_data_batch = test_data[batch_indices, :, :, :, :]
                    test_data_batch_dev = torch.from_numpy(test_data_batch).float().to(device)
                    out = net(test_data_batch_dev)[0]

                    out = out.data.cpu().numpy().reshape(-1)
                    prediction[batch_indices] = out

                # Compute performance metrics
                if args.regression:
                    test_mae = mean_absolute_error(prediction, test_label)
                    print(f'R^2: {r2_score(prediction, test_label):.4f}')
                    print(f'r: {pearsonr(prediction, test_label)}')
        ...
        if args.regression:
            print(f"Best MAE = {best_test_mae_curr_fold} "
                    f"at epoch {best_test_epoch_curr_fold}")
            results.append([
                network, '_'.join(args.ensemble_networks), best_test_mae_curr_fold, best_test_epoch_curr_fold
            ])
        else:
            print(f"Best AUC = {best_test_auc_curr_fold} "
                    f"at epoch {best_test_epoch_curr_fold}")
            results.append([
                network, '_'.join(args.ensemble_networks), best_test_auc_curr_fold, best_test_epoch_curr_fold
            ])

    # After finishing all folds for a given network, save results
    if args.regression:
        results_df = pd.DataFrame(results, columns=[
            'Network', 'Ensemble Network', 'Best MAE', 'Epoch'
        ])
    else:
        results_df = pd.DataFrame(results, columns=[
            'Network', 'Ensemble Network', 'Best AUC', 'Epoch'
        ])

    # Save results and predictions
    results_file = f'training_results_{network.strip()}.csv'
    preds_file   = f'predictions_{network.strip()}.csv'
