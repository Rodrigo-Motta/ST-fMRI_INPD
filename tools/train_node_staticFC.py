...
def train(args):
    ...
    for network in networks:
        ...
        for fold in range(1, 6):
            ...
            if args.regression:
                print(f"Best MAE for fold {fold} = {best_test_mae_curr_fold} "
                      f"at epoch {best_test_epoch_curr_fold}")
                results.append([
                    network, '_'.join(args.ensemble_networks), 
                    fold, best_test_mae_curr_fold, best_test_epoch_curr_fold
                ])
            else:
                print(f"Best AUC for fold {fold} = {best_test_auc_curr_fold} "
                      f"at epoch {best_test_epoch_curr_fold}")
                results.append([
                    network, '_'.join(args.ensemble_networks), 
                    fold, best_test_auc_curr_fold, best_test_epoch_curr_fold
                ])

        # After finishing all folds for a given network, save results
        if args.regression:
            results_df = pd.DataFrame(results, columns=[
                'Network', 'Ensemble Network', 'Fold', 'Best MAE', 'Epoch'
            ])
        else:
            results_df = pd.DataFrame(results, columns=[
                'Network', 'Ensemble Network', 'Fold', 'Best AUC', 'Epoch'
            ])
